...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if applied > 1:
        raise SystemExit(
            f"ERROR: {label}: duplicate patched result found {applied} times; no file written"
        )
...
def main() -> int:
    ...
    if "static const char *const kFovPreset[]" in text:
        print("OK: preset names already applied")
    else:
        text = replace_once(
            text,
    ...
    if '.key="Ascension.FovPreset"' in text:
        print("OK: FOV preset row already applied")
    else:
    ...
    if '.key="Ascension.MousePreset"' in text and '.key="Ascension.GamepadPreset"' in text:
        print("OK: mouse/gamepad preset rows already applied")
    else:
    ...
    if "static int  s_fovPreset = 0;" in text:
        print("OK: preset state already applied")
    else:
    ...
    if 'configRegisterInt("Ascension.FovPreset"' in text:
        print("OK: preset config registration already applied")
    else:
