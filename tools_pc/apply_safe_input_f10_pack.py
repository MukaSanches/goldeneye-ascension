...
def patch_input() -> None:
    ...
    if "static int wheelWeaponCycle = 1;" in text:
        print("OK: wheel-cycle option storage already applied")
    else:
        text = replace_once(
            text,
    ...
    if 'configRegisterInt("Input.MouseWheelWeaponCycle"' in text:
        print("OK: wheel-cycle config already applied")
    else:
        text = replace_once(
            text,
...
def patch_overlay() -> None:
    ...
    if '.key="Input.MouseWheelWeaponCycle"' in text:
        print("OK: safe existing input rows + wheel toggle already applied")
    else:
