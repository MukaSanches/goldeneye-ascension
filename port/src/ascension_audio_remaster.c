...
/* Steam Audio's built-in 4.8.1 HRTF contains 44.1, 48 and 24 kHz data, but
 * GoldenEye renders at 22.05 kHz. 24 kHz is the closest native HRTF set.
 * Processing the GE samples against this filter bank preserves the game's
 * sample clock/pitch and avoids the hard initialization failure at 22.05 kHz.
 * The 8.8% HRTF spectral-scale offset is confined to spatial coloration; the
 * legacy mix remains the sample-accurate fallback if Steam Audio ever fails. */
#define ASC_STEAM_HRTF_RATE 24000
...
{
    if (fifo->count >= ASC_AUDIO_FIFO_FRAMES) return 0;
...
{
    if (!message) return;
    if (level == ASC_IPL_LOGLEVEL_ERROR) sysLogPrintf(LOG_ERROR, "Steam Audio: %s", message);
    else if (level == ASC_IPL_LOGLEVEL_WARNING) sysLogPrintf(LOG_NOTE, "Steam Audio: %s", message);
    else if (getenv("GE_ASCENSION_AUDIO_DIAG")) sysLogPrintf(LOG_NOTE, "Steam Audio: %s", message);
}

static int loadFunction(const char *name, void *destination, size_t destinationSize)
{
    void *symbol;
    if (!s_audio.library || destinationSize != sizeof(symbol)) return 0;
...
static void *loadSteamAudioLibrary(void)
{
    ...
    if (!lib) lib = SDL_LoadObject(runtimeName);
    if (!lib) lib = SDL_LoadObject(repoPath);
    return lib;
}

static ASC_IPLSIMDLevel bestSimdLevel(void)
{
    if (SDL_HasAVX2()) return ASC_IPL_SIMDLEVEL_AVX2;
    if (SDL_HasAVX()) return ASC_IPL_SIMDLEVEL_AVX;
    if (SDL_HasSSE42()) return ASC_IPL_SIMDLEVEL_SSE4;
...
static void disableSourceHrtf(void)
{
    ...
    if (s_audio.sourceHrtf && s_audio.hrtfRelease) s_audio.hrtfRelease(&s_audio.sourceHrtf);
...
{
    disableSourceHrtf();
    if (s_audio.effect && s_audio.effectRelease) s_audio.effectRelease(&s_audio.effect);
    if (s_audio.hrtf && s_audio.hrtfRelease) s_audio.hrtfRelease(&s_audio.hrtf);
    if (s_audio.context && s_audio.contextRelease) s_audio.contextRelease(&s_audio.context);
    if (s_audio.library) { SDL_UnloadObject(s_audio.library); s_audio.library = NULL; }
    s_audio.steamActive = 0;
}

static int loadBinauralApi(void)
{
#define LOAD_SOURCE(member, symbol) do { \
    if (!loadFunction(symbol, &s_audio.member, sizeof(s_audio.member))) { \
        sysLogPrintf(LOG_NOTE, "Ascension Audio: optional Steam Audio source symbol missing: %s; post-mix fallback retained", symbol); \
        s_audio.binauralCreate = NULL; s_audio.binauralRelease = NULL; s_audio.binauralReset = NULL; \
        s_audio.binauralApply = NULL; s_audio.binauralGetTail = NULL; s_audio.binauralGetTailSize = NULL; return 0; \
    } } while (0)
...
static int initSourceHrtf(void)
{
    ...
    if (sourceMode && sourceMode[0] == '0' && sourceMode[1] == '\0') {
        sysLogPrintf(LOG_NOTE, "Ascension Audio: per-source HRTF disabled by GE_ASCENSION_SOURCE_HRTF=0; post-mix fallback active");
        return 0;
    }
    if (!loadBinauralApi()) return 0;
    audioSettings.samplingRate = ASC_STEAM_HRTF_RATE;
    ...
    if (s_audio.hrtfCreate(s_audio.context, &audioSettings, &hrtfSettings, &s_audio.sourceHrtf) != ASC_IPL_STATUS_SUCCESS || !s_audio.sourceHrtf) {
        sysLogPrintf(LOG_NOTE, "Ascension Audio: per-source HRTF creation failed; post-mix fallback retained");
        disableSourceHrtf();
        return 0;
    }
    ...
    for (i = 0; i < ASC_SOURCE_EFFECTS; ++i) {
        if (s_audio.binauralCreate(s_audio.context, &audioSettings, &effectSettings, &s_audio.sourceEffects[i].effect) != ASC_IPL_STATUS_SUCCESS || !s_audio.sourceEffects[i].effect) {
            sysLogPrintf(LOG_NOTE, "Ascension Audio: per-source HRTF pool creation failed at slot %u; post-mix fallback retained", i);
            disableSourceHrtf();
            return 0;
        }
    }
    s_audio.sourceActive = 1;
    sysLogPrintf(LOG_NOTE, "Ascension Audio: per-source Steam Audio HRTF active (%u voices, %u-sample frames, bilinear, native HRTF %d Hz)", (unsigned)ASC_SOURCE_EFFECTS, (unsigned)ASCENSION_AUDIO_SOURCE_FRAME, ASC_STEAM_HRTF_RATE);
    return 1;
}
...
static int initSteam(void)
{
    ...
    if (!s_audio.library) {
        sysLogPrintf(LOG_NOTE, "Ascension Audio: Steam Audio runtime not found; mastering fallback active (%s)", SDL_GetError());
        return 0;
    }
#define LOAD_REQUIRED(member, symbol) do { \
    if (!loadFunction(symbol, &s_audio.member, sizeof(s_audio.member))) { \
        sysLogPrintf(LOG_ERROR, "Ascension Audio: Steam Audio symbol missing: %s", symbol); releaseSteam(); return 0; \
    } } while (0)
...
#undef LOAD_REQUIRED
    ...
    if (s_audio.contextCreate(&contextSettings, &s_audio.context) != ASC_IPL_STATUS_SUCCESS || !s_audio.context) {
        sysLogPrintf(LOG_ERROR, "Ascension Audio: Steam Audio context creation failed"); releaseSteam(); return 0;
    }
    audioSettings.samplingRate = ASC_STEAM_HRTF_RATE;
    ...
    if (s_audio.hrtfCreate(s_audio.context, &audioSettings, &hrtfSettings, &s_audio.hrtf) != ASC_IPL_STATUS_SUCCESS || !s_audio.hrtf) {
        sysLogPrintf(LOG_ERROR, "Ascension Audio: Steam Audio HRTF creation failed"); releaseSteam(); return 0;
    }
    memset(&effectSettings, 0, sizeof(effectSettings));
    ...
    if (s_audio.effectCreate(s_audio.context, &audioSettings, &effectSettings, &s_audio.effect) != ASC_IPL_STATUS_SUCCESS || !s_audio.effect) {
        sysLogPrintf(LOG_ERROR, "Ascension Audio: Steam Audio virtual-surround creation failed"); releaseSteam(); return 0;
    }
    s_audio.inPlanes[0] = s_audio.inL; s_audio.inPlanes[1] = s_audio.inR;
    s_audio.outPlanes[0] = s_audio.wetL; s_audio.outPlanes[1] = s_audio.wetR;
...
static SourceEffectSlot *sourceSlot(uint32_t voiceKey)
{
    ...
    for (i = 0; i < ASC_SOURCE_EFFECTS; ++i) {
        ...
        if (slot->assigned && slot->voiceKey == voiceKey) { slot->lastUse = ++s_audio.sourceSerial; return slot; }
        if (!slot->assigned) { oldest = slot; break; }
        if (!oldest || slot->lastUse < oldest->lastUse) oldest = slot;
    }
    if (!oldest || !oldest->effect) return NULL;
    s_audio.binauralReset(oldest->effect);
    oldest->voiceKey = voiceKey; oldest->assigned = 1; oldest->lastUse = ++s_audio.sourceSerial;
    return oldest;
}
...
{
    unsigned i;
    if (!ascensionAudioSourceHrtfActive() || !s_audio.binauralReset) return;
    for (i = 0; i < ASC_SOURCE_EFFECTS; ++i) {
        SourceEffectSlot *slot = &s_audio.sourceEffects[i];
        if (slot->assigned && slot->voiceKey == voiceKey) { s_audio.binauralReset(slot->effect); slot->lastUse = ++s_audio.sourceSerial; return; }
    }
}

int ascensionAudioSourceProcess(uint32_t voiceKey, const float *mono, float directionX, float directionY, float directionZ, float *outLeft, float *outRight)
{
    SourceEffectSlot *slot;
    ASC_IPLBinauralEffectParams params;
    ASC_IPLAudioBuffer inBuffer, outBuffer;
    float *inPlanes[1], *outPlanes[2];
    float lengthSquared, invLength;
    unsigned i;
    if (!ascensionAudioSourceHrtfActive() || !mono || !outLeft || !outRight || !s_audio.binauralApply || !s_audio.binauralReset) return 0;
    lengthSquared = directionX * directionX + directionY * directionY + directionZ * directionZ;
    if (!isfinite(lengthSquared) || lengthSquared < 1.0e-12f) return 0;
    invLength = 1.0f / sqrtf(lengthSquared);
    directionX *= invLength; directionY *= invLength; directionZ *= invLength;
    slot = sourceSlot(voiceKey);
    if (!slot) return 0;
    inPlanes[0] = (float *)mono; outPlanes[0] = outLeft; outPlanes[1] = outRight;
    inBuffer.numChannels = 1; inBuffer.numSamples = (ASC_IPLint32)ASCENSION_AUDIO_SOURCE_FRAME; inBuffer.data = inPlanes;
    outBuffer.numChannels = 2; outBuffer.numSamples = (ASC_IPLint32)ASCENSION_AUDIO_SOURCE_FRAME; outBuffer.data = outPlanes;
    memset(&params, 0, sizeof(params));
    params.direction.x = directionX; params.direction.y = directionY; params.direction.z = directionZ;
    params.interpolation = ASC_IPL_HRTFINTERPOLATION_BILINEAR; params.spatialBlend = 1.0f; params.hrtf = s_audio.sourceHrtf; params.peakDelays = NULL;
    s_audio.binauralApply(slot->effect, &params, &inBuffer, &outBuffer);
    for (i = 0; i < ASCENSION_AUDIO_SOURCE_FRAME; ++i) {
        if (!isfinite(outLeft[i]) || !isfinite(outRight[i]) || fabsf(outLeft[i]) > 8.0f || fabsf(outRight[i]) > 8.0f) {
            s_audio.binauralReset(slot->effect); return 0;
        }
    }
    return 1;
}
...
static float spectralChannel(float x, int channel)
{
    ...
    s_audio.dcPrevIn[channel] = x; s_audio.dcPrevOut[channel] = dc;
    ...
    return dc + 0.045f * s_audio.lowState[channel] + 0.050f * (dc - s_audio.highLPState[channel]);
}
...
{
    float l = spectralChannel(*left, 0), r = spectralChannel(*right, 1);
    float mid = 0.5f * (l + r), side = 0.5f * (l - r) * 1.035f;
    *left = softPeak(mid + side); *right = softPeak(mid - side);
}
...
static void processSteamFrames(void)
{
    while (s_audio.inputFifo.count >= ASC_AUDIO_FRAME) {
        ...
        unsigned i;
        for (i = 0; i < ASC_AUDIO_FRAME; ++i) { float l = 0.0f, r = 0.0f; fifoPop(&s_audio.inputFifo, &l, &r); s_audio.inL[i] = l; s_audio.inR[i] = r; }
        inBuffer.numChannels = 2; inBuffer.numSamples = ASC_AUDIO_FRAME; inBuffer.data = s_audio.inPlanes;
        outBuffer.numChannels = 2; outBuffer.numSamples = ASC_AUDIO_FRAME; outBuffer.data = s_audio.outPlanes;
        ...
        for (i = 0; i < ASC_AUDIO_FRAME; ++i) {
            ...
            if (!fifoPush(&s_audio.outputFifo, l, r)) {
                unsigned j; memset(&s_audio.outputFifo, 0, sizeof(s_audio.outputFifo));
                for (j = 0; j < ASC_AUDIO_FRAME; ++j) fifoPush(&s_audio.outputFifo, 0.0f, 0.0f);
                return;
            }
        }
    }
...
int ascensionAudioRemasterInit(int sampleRate)
{
    ...
    if (s_audio.bypass) { sysLogPrintf(LOG_NOTE, "Ascension Audio: developer bypass active"); return 0; }
    initSteam();
    if (s_audio.steamActive && !s_audio.sourceActive) {
        for (i = 0; i < ASC_AUDIO_FRAME; ++i) fifoPush(&s_audio.outputFifo, 0.0f, 0.0f);
        sysLogPrintf(LOG_NOTE, "Ascension Audio Remaster: Steam Audio 4.8.1 post-mix HRTF fallback active (GE %d Hz, HRTF %d Hz, %d-frame DSP)", s_audio.sampleRate, ASC_STEAM_HRTF_RATE, ASC_AUDIO_FRAME);
    } else if (s_audio.sourceActive) {
        sysLogPrintf(LOG_NOTE, "Ascension Audio Remaster: source-object HRTF + mastering active (GE %d Hz, HRTF %d Hz, zero extra post-mix FIFO latency)", s_audio.sampleRate, ASC_STEAM_HRTF_RATE);
    } else sysLogPrintf(LOG_NOTE, "Ascension Audio Remaster: mastering fallback active");
...
void ascensionAudioRemasterShutdown(void)
{
    ...
    releaseSteam(); memset(&s_audio, 0, sizeof(s_audio));
}
...
{
    unsigned frames, i;
    if (!stereoS16 || !lenBytes || s_audio.bypass || !s_audio.initialized) return stereoS16;
    frames = lenBytes / (sizeof(int16_t) * 2u);
    if (!frames || frames > ASC_AUDIO_MAX_CALL_FRAMES) return stereoS16;
    if (s_audio.sourceActive || !s_audio.steamActive) {
        for (i = 0; i < frames; ++i) {
            float l = (float)stereoS16[i * 2u] / 32768.0f, r = (float)stereoS16[i * 2u + 1u] / 32768.0f;
            masterPair(&l, &r); s_audio.returnS16[i * 2u] = floatToS16(l); s_audio.returnS16[i * 2u + 1u] = floatToS16(r);
        }
        return s_audio.returnS16;
    }
    for (i = 0; i < frames; ++i) {
        float l = (float)stereoS16[i * 2u] / 32768.0f, r = (float)stereoS16[i * 2u + 1u] / 32768.0f;
        if (!fifoPush(&s_audio.inputFifo, l, r)) return stereoS16;
    }
    processSteamFrames();
    for (i = 0; i < frames; ++i) {
        float l = 0.0f, r = 0.0f;
        if (!fifoPop(&s_audio.outputFifo, &l, &r)) return stereoS16;
        s_audio.returnS16[i * 2u] = floatToS16(l); s_audio.returnS16[i * 2u + 1u] = floatToS16(r);
    }
    return s_audio.returnS16;
}
