...
/* Serialized N64 sizes. These mirror sizepropdef(), not host sizeof(). */
...
static AscSemanticKind prop_kind(uint8_t type)
{
    ...
    if (type == 8 || type == 7 || type == 20) return ASC_SEM_WEAPON;
    ...
    if (type == 14 || type == 18 || type == 19 || type == 37) return ASC_SEM_PRESET;
...
static void decode_object_fields(const uint8_t *p, uint32_t n, AscSemanticNode *node)
{
    ...
    if (type == 1 && n >= 0xa8) {
        ...
        node->values[1] = (int16_t)be16(p + 6); /* command-relative object offset */
    } else if (type == 14 || type == 19 || type == 38) {
        if (n >= 12) {
        ...
        node->values[1] = (int32_t)be32(p + 12); /* difficulty */
...
static int decode_fixed_terminated(const AscSetupSection *s, AscSemanticDocument *out,
                                   AscSemanticKind kind, uint32_t stride,
                                   uint32_t terminator_offset, uint32_t id_base)
{
    uint32_t off = 0, index = 0;
    while (off + stride <= s->payload.size) {
        const uint8_t *p = s->payload.data + off;
        AscSemanticNode node;
        if (be32(p + terminator_offset) == 0) break;
        memset(&node, 0, sizeof(node));
        node.kind = kind;
        node.stable_id = id_base | index;
        node.source_offset = s->source_offset + off;
        node.byte_size = stride;
        node.refs[0] = be32(p + terminator_offset);
        if (kind == ASC_SEM_PATH && stride >= 8) {
            node.values[0] = p[4];
            node.values[1] = p[5];
            node.values[2] = be16(p + 6);
        } else if (kind == ASC_SEM_WAYPOINT && stride >= 16) {
            node.values[0] = (int32_t)be32(p + 0);
            node.refs[0] = be32(p + 4);
            node.values[1] = (int32_t)be32(p + 8);
            node.values[2] = (int32_t)be32(p + 12);
        } else if (kind == ASC_SEM_WAYGROUP && stride >= 12) {
            node.refs[0] = be32(p + 0);
            node.refs[1] = be32(p + 4);
            node.values[0] = (int32_t)be32(p + 8);
        } else if (kind == ASC_SEM_AI_SCRIPT && stride >= 8) {
            node.refs[0] = be32(p + 0);
            node.values[0] = (int32_t)be32(p + 4); /* list ID */
        } else if ((kind == ASC_SEM_PAD || kind == ASC_SEM_BOUND_PAD) && stride >= 40) {
            node.refs[0] = be32(p + 36); /* plink */
            node.refs[1] = be32(p + 40); /* runtime stan slot, retained */
            node.values[0] = (int32_t)be32(p + 0);  /* position float bits */
                        ...
                        AscSetupDiagnostics *diagnostics)
{
    ...
    for (i = 0; i < setup->section_count; ++i) {
        ...
        case ASC_SETUP_SECTION_PATH_TABLE:
            status = decode_fixed_terminated(s, out, ASC_SEM_WAYPOINT, 16, 4, 0x30000000u); break;
        case ASC_SETUP_SECTION_PATH_LINKS:
            status = decode_fixed_terminated(s, out, ASC_SEM_WAYGROUP, 12, 0, 0x31000000u); break;
        case ASC_SETUP_SECTION_PATHS:
            status = decode_fixed_terminated(s, out, ASC_SEM_PATH, 8, 0, 0x32000000u); break;
        case ASC_SETUP_SECTION_AI_LISTS:
            status = decode_fixed_terminated(s, out, ASC_SEM_AI_SCRIPT, 8, 0, 0x40000000u); break;
        case ASC_SETUP_SECTION_PADS:
            status = decode_fixed_terminated(s, out, ASC_SEM_PAD, 44, 36, 0x50000000u); break;
        case ASC_SETUP_SECTION_PADS_3D:
            status = decode_fixed_terminated(s, out, ASC_SEM_BOUND_PAD, 68, 36, 0x51000000u); break;
        default:
            break;
        }
